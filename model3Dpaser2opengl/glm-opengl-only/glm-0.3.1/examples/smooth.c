...
void print2file(GLMmodel *model){

    FILE *pFile=NULL;
    char tmpout[128];
    int i = 0;
    sprintf(tmpout, "model3d.h");
    ...
    fflush(pFile);
    fprintf(pFile, "unsigned int modelNumVerts = %d; \n\n", model->numvertices);

    fprintf(pFile, "float modelVerts [] = { \n");
    ...
    fprintf(pFile, "unsigned int modelNumNormals = %d; \n\n", model->numnormals);
    fprintf(pFile, "float modelNormals [] = { \n");
    ...
    fprintf(pFile, "unsigned int modelNumTexCoords = %d; \n\n", model->numtexcoords);
    fprintf(pFile, "float modelsTexCoords [] = { \n");
...
keyboard(unsigned char key, int x, int y)
{
	...
	glmWriteOBJ(model, "out.obj", GLM_SMOOTH | GLM_MATERIAL | GLM_TEXTURE);
	break;
